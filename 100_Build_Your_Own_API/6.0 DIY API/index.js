...
app.put("/jokes/:id" , (req, res)=>{
  const id = Number(req.params.id);
  if(Number.isNaN(id) || id <= 0 || id > jokes.length){
    return res.sendStatus(404)
  }
  const {jokeText, jokeType} = req.body;

  jokes[id-1].jokeText = jokeText;
  jokes[id-1].jokeType = jokeType;
...
app.patch("/jokes/:id" , (req, res)=>{
  const id = Number(req.params.id);
  if (Number.isNaN(id) || id < 1 || id > jokes.length) {
    return res.sendStatus(404);
  }
...
app.delete("/jokes/:id", (req, res)=>{
  ...
  const deleted_joke = jokes.splice(jokes[id-1], 1)

  res.json(deleted_joke[0])
